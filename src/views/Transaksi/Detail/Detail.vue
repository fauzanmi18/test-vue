...
let realDetailId
if(localStorage.getItem('detailId')) realDetailId = localStorage.getItem('detailId')
else router.push({name: '404notfound'})

if(isNaN(paramId)){
    router.push({name: '404notfound'})
} else {
    if(paramId !== realDetailId) router.push({name: '404notfound'})
}

const detailDataPesanan = ref(null)

onBeforeMount(() => {
    const dataDetailPesanan = JSON.parse(localStorage.getItem('dataDetailPesanan'));

    if (dataDetailPesanan) {
        console.log(dataDetailPesanan.payment_status);
        
        if (dataDetailPesanan.payment_status.toLowerCase() !== 'telah dibayar') {
            router.push({ name: '404notfound' });
        } else {
            detailDataPesanan.value = dataDetailPesanan;
        }
    } else {
        router.push({ name: '404notfound' });
    }
});
// console.log(detailDataPesanan)
...
<template>
    ...
    <div class="mx-auto max-w-270">
      <!-- Breadcrumb Start -->
        <BreadcrumbDefault pageTitle="Detail Pesanan" />
        <!-- Breadcrumb End -->
        <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            <div class="border-b border-stroke px-4 py-4 dark:border-strokedark sm:px-6 xl:px-7.5"><h3 class="font-medium text-black dark:text-white">
                Detail Transaksi
            </h3></div>
            <div class="p-4 sm:p-6 xl:p-9">
                <div class="mb-10 flex flex-wrap items-center justify-end gap-3.5">
                    <!-- <button class="inline-flex items-center gap-2.5 rounded bg-meta-3 px-4 py-[7px] font-medium text-white hover:bg-opacity-90">
                    ...
                    <button v-if="loadingConvert == false" @click="convertPDF()" class="inline-flex items-center gap-2.5 rounded bg-primary px-4 py-[7px] font-medium text-white hover:bg-opacity-90">
                        <svg class="fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                    ...
                    <button v-if="loadingConvert == true" @click="convertPDF()" class="inline-flex items-center gap-2.5 rounded bg-primary px-4 py-[7px] font-medium text-white hover:bg-opacity-90">
                        Loading...
                    </button>
                </div>
                <div v-if="detailDataPesanan" id="toPDF" class="w-full">
                    <div class="flex flex-wrap justify-between gap-5"> 
                        <div>
                            <p class="mb-1.5 font-medium text-black dark:text-teal-600">Ordered By:</p>
                            <h4 class="mb-3 text-xl font-bold text-black dark:text-teal-600">
                                {{ detailDataPesanan.user.users_details.first_name }} {{ detailDataPesanan.user.users_details.last_name }}
                            </h4>
                            <a href="#" class="block"><span class="font-medium text-black dark:text-teal-600">
                                Email:</span>
                                {{ detailDataPesanan.user.email }}
                                </a>
                            <a href="#" class="block"><span class="font-medium text-black dark:text-teal-600">
                                Phone:</span>
                                {{ detailDataPesanan.user.users_details.number }}
                                </a>
                            <span class="mt-1.5 block"><span class="font-medium text-black dark:text-teal-600">
                                Address:</span> {{ detailDataPesanan.shipment_address }}
                                </span>
                        </div>
                        <div>
                            <p class="mb-1.5 font-medium text-black dark:text-teal-600">Invoice ID:</p>
                            <h4 class="mb-3 text-xl font-bold text-black dark:text-teal-600">{{ detailDataPesanan.payments.merchant_order_id }}</h4>
                            <a href="#" class="block"><span class="font-medium text-black dark:text-teal-600">
                                Issued Date:</span> {{ detailDataPesanan.created_at }}
                                </a>
                        </div>
                    </div>
                    <!-- <div class="my-7.5 grid grid-cols-1 border border-stroke dark:border-strokedark xsm:grid-cols-2 sm:grid-cols-4">
                        <div class="border-b border-r border-stroke px-5 py-4 last:border-r-0 dark:border-strokedark sm:border-b-0">
                            ...
                            <span class="text-sm font-medium"> {{ detailDataPesanan.payments.merchant_order_id }} </span>
                        </div>
                    ...
                    <div class="my-7.5 border border-stroke dark:border-strokedark">
                        <div class="max-w-full overflow-x-auto">
                            <div class="min-w-[670px]">
                                <div class="grid grid-cols-12 border-b border-stroke py-3.5 pl-5 pr-6 dark:border-strokedark">
                                    <div class="col-span-3"><h5 class="font-medium text-black dark:text-teal-600">Product</h5></div>
                                    <div class="col-span-4"><h5 class="font-medium text-black dark:text-teal-600">Description</h5></div>
                                    <div class="col-span-2 ml-3"><h5 class="font-medium text-black dark:text-teal-600">Quantity</h5></div>
                                    <div class="col-span-2"><h5 class="font-medium text-black dark:text-teal-600">Price Per Unit</h5></div>
                                    <div class="col-span-1"><h5 class="text-right font-medium text-black dark:text-teal-600">Total</h5></div>
                                </div>
                                <div v-for="(item, index) in detailDataPesanan.users_carts" :key="index" class="grid grid-cols-12 border-b border-stroke py-3.5 pl-5 pr-6 dark:border-strokedark">
                                    <div class="col-span-3"><p class="text-base font-medium">{{ item.products.name }}</p></div>
                                    <div class="col-span-4"><p class="text-base font-medium">{{ item.products.description }}</p></div>
                                    <div class="col-span-2 ml-3"><p class="text-base font-medium">{{ item.qty }} Pcs</p></div>
                                    <div class="col-span-2"><p class="text-base font-medium">{{ formatRupiah(item.products.price) }}</p></div>
                                    <div class="col-span-1"><p class="text-right text-base font-medium">{{ formatRupiah(item.total_price) }}</p></div>
                                </div>
                            </div>
                        </div>
                        <div class="flex justify-end p-6">
                            <div class="w-full max-w-65">
                                <div class="flex flex-col gap-4">
                                    <p class="flex justify-between font-medium text-sm text-black dark:text-teal-600"><span> 
                                        Subtotal </span><span> {{ detailDataPesanan.total_product_price }} </span>
                                        </p>
                                    <p class="flex justify-between font-medium text-sm text-black dark:text-teal-600"><span> 
                                        Shipping Cost (+) </span><span> {{ detailDataPesanan.ongkir }} </span>
                                        </p>
                                    <!-- <p class="flex justify-between font-medium text-black dark:text-white">
                                        <span> Coupon Discount <span class="text-meta-3">(10%)</span></span><span> $470 </span>
                                    </p>
                                    <p class="flex justify-between font-medium text-black dark:text-white">
                                        <span> Vat <span class="text-red">(5%)</span></span><span> $235 </span>
                                    </p> -->
                                </div>
                                <p class="mt-4 flex justify-between border-t border-stroke pt-5 dark:border-strokedark"><span class="font-medium text-black dark:text-teal-600"> 
                                    Total </span><span class="font-bold text-meta-3"> {{ detailDataPesanan.total_price }} </span></p>
                                <!-- <button class="float-right mt-10 inline-flex items-center gap-2.5 rounded bg-primary px-7.5 py-2.5 font-medium text-white hover:bg-opacity-90">
                                    Download
                                    <svg class="fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <defs>
                                            <clipPath id="clip0_1878_13706"><rect width="18" height="18" fill="white"></rect></clipPath>
                                        </defs>
                                    </svg>
                                </button> -->
                            </div>
