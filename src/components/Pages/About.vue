...
const getDataUser = async() => {
  try {
    ...
    const response = await initAPI('get', 'pages?page_type=about&status=1', null, token)
...
const btnAction = async(type, id) => {
  ...
  formData.append('title', DOMPurify.sanitize(title.value))
  formData.append('content', DOMPurify.sanitize(konten.value))
  selectedImages.value.forEach((image, index) => {
    // console.log('Appending image file:', image.file)
...
<template>
  ...
  <div v-if="about.length == 0 && !isTambahKonten">
    ...
    >
      <div class="flex flex-col py-4 justify-center items-center">
        <h1 class="mb-4 font-medium text-black dark:text-white">Anda belum menambah konten halaman About.</h1>
  ...
  <div v-if="about.length > 0 || isTambahKonten" class="flex items-start gap-2">
    <div
      class="w-1/2 min-h-[340px] rounded-sm border border-stroke bg-white px-5 pt-4 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1"
    >
      ...
      <div :class="{'gap-2': about.length > 0}" class="flex justify-end mb-4">
        <button @click="btnAction('tambah')" v-if="!loadingSubmit && isTambahKonten && about.length == 0" class="text-white font-medium bg-primary px-4 py-2 rounded-lg">
          Tambah
        </button>
        <button @click="btnAction('edit', about[0].id)" v-if="!loadingSubmit && (about.length > 0 || !isTambahKonten)" class="text-white font-medium bg-primary px-4 py-2 rounded-lg">
          Edit
        </button>
    ...
    <div
      class="w-1/2 max-h-[383px] overflow-y-auto rounded-sm border border-stroke bg-white px-5 pt-4 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1"
    >
      <div class="flex flex-col gap-2">
        <div class="mb-4.5 font-medium">
          <div class="mb-4 sticky -top-4 bg-white dark:bg-boxdark flex justify-between items-center">
            ...
            <button v-if="hasImage" @click="selectedImages.length = 0, hasImage = false" class="text-white text-xs font-medium bg-primary w-[126px] h-[30px] rounded-lg">
              Pilih ulang gambar
            </button>
          </div>
