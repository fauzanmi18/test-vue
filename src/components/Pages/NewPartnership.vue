...
import Modal from '@/components/Modal/Partnership/Partnership.vue'
...
const getData = async(page = 1, query = '') => {
  try {
    ...
    let endpoint = query !== '' 
      ? `pages?page_type=partnership&status=1&page=${page}&search=${query}`
      : `pages?page_type=partnership&status=1&page=${page}`
...
const deletePartnership = async() => {
  try {
    ...
    const response = await initAPI('delete', 'pages/'+deletePartnershipId.value.id, null, token)
...
<template>
  ...
  modalTitle="Hapus partnership ini?"
        ...
        <tbody>
          ...
          <tr v-else v-for="(item, index) in categories" :key="index">
            ...
            <td class="py-5 px-4 pl-9 xl:pl-11">
              <img loading="lazy" class="w-[54px] h-[54px]" :src="baseImgUrl+item.images[0]">
            </td>
            <td class="py-5 px-4">
              <h5 class="font-medium text-black dark:text-white">
                {{ item.title }}
