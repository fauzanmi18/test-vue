...
import Modal from '@/components/Modal/Blogs/Blogs.vue'
...
const getData = async(page = 1, query = '') => {
  try {
    ...
    blogsMisc.value.totalHalaman = response.data.last_page
    blogsMisc.value.itemsPerPage = response.data.per_page
    blogsMisc.value.currPage = response.data.current_page
    blogsMisc.value.nextPage = response.data.next_page_url
    blogsMisc.value.prevPage = response.data.prev_page_url
    blogsMisc.value.lastPage = response.data.last_page
    blogsMisc.value.totalDari = response.data.from
    blogsMisc.value.totalKe = response.data.to
    blogsMisc.value.totalData = response.data.total
  } catch (error) {
...
const deletePartnershipId = ref({
  id: '',
})

const btnAction = (id, method) => {
  if(method == 'edit'){
    const unik = blogs.value.find(item => item.id === id)
    console.log(`euy`, unik)
    detailData.value = unik
    methodType.value = method
    showModal()
  } else {
    deletePartnershipId.value.id = id
    methodType.value = method
    toggleModalDelete()
  } 
}
...
const deletePartnership = async() => {
  try {
    ...
    const response = await initAPI('delete', 'blogs/'+deletePartnershipId.value.id, null, token)
    console.log(response.data)
    toggleNotif('Success', response.data.message)
    getData()
  } catch (error) {
    console.log(error)
    toggleNotif('Error', 'Terjadi kesalahan saat hapus partnership')
  } finally {
    getData()
  }
}
...
<template>
  ...
  <DialogDelete 
  @toggleDialog="toggleModalDelete" @action="deletePartnership" v-if="isModalDelete"
  modalTitle="Hapus blog ini?"
  modalMessage="Data yang dihapus tidak dapat dikembalikan."
  :method="methodType" 
  />
      ...
      <table class="max-w-full overflow-x-auto table-auto">
        <thead>
          <tr class="bg-gray-2 text-left dark:bg-meta-4">
            ...
            <th class="min-w-fit py-4 px-4 font-medium text-black dark:text-white xl:pl-11">
              Gambar
            </th>
        ...
        <tbody>
          ...
          <tr v-else v-for="(item, index) in blogs" :key="index">
            <td class="py-5 px-4">
              <h5 class="font-medium text-black dark:text-white">
                {{ (blogsMisc.currPage - 1) * blogsMisc.itemsPerPage + index + 1 }}
              </h5>
            </td>
            <td class="py-5 px-4 pl-9 xl:pl-11">
                <img v-if="item.image.length > 0" loading="lazy" class="w-[54px] h-[54px]" :src="baseImgUrl+item.image[0]">
                <span v-else>No Image</span>
            </td>
            <td class="py-5 px-4 pl-11">
              <h5 class="font-medium text-black dark:text-white">
                {{ item.title }}
              </h5>
            </td>
  ...
  <div v-if="blogs.length > 0" class="mt-4 flex justify-between items-center rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
    <Pagination
      :currentPage="blogsMisc.currPage" 
      :lastPage="blogsMisc.lastPage"
      :prevPageUrl="blogsMisc.prevPage"
      :nextPageUrl="blogsMisc.nextPage"
      :query="searchQuery" 
      @changePage="getData"
    />
    <span class="text-sm text-black dark:text-white font-medium">
      {{ blogsMisc.totalKe }} Data dari {{ blogsMisc.totalData }} total data.
      </span>
  </div>
</template>
