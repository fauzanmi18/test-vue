                ...
                <div class="w-full p-4 h-[480px] overflow-y-scroll">
                    <div class="w-full flex flex-col justify-center">
                        <div class="flex flex-col mb-4">
                            ...
                            <div v-if="selectedImage.length > 0" class="flex bg-red max-w-[640px] overflow-x-scroll gap-2 mb-2 justify-center">
                                <img v-for="(image, index) in selectedImage" :key="index" :src="image.url" class="w-[120px] h-[120px]px-2" alt="Foto Produk">
                            </div>
                            <img v-if="selectedImage.length == 0" class="mb-2 self-center w-[120px] h-[120px]" src="https://placehold.co/454x454" alt="Foto Produk">
                            <!---<img v-if="selectedImage" :src="selectedImage" class="mb-2 self-center w-[120px] h-[120px]" alt="Foto Produk"> -->
                            <input v-if="$props.method !== 'detail'" @change="onFileChange" multiple type="file" class="w-full text-xs rounded-lg border border-stroke bg-transparent py-2 pl-6 pr-10 outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary text-black dark:text-white">
                        </div>
...
const onFileChange = (event) => {
    ...
    files.forEach(file => {
        ...
        selectedImage.value.push({ url, file });
    });
};
