                ...
                <div class="w-full p-4 h-[380px]">
                    <div class="w-full flex flex-col gap-4 justify-center">
                        <div class="flex flex-col mb-4">
                            ...
                            <img v-if="!selectedImage" class="mb-2 self-center w-[120px] h-[120px]" src="https://placehold.co/184x100" alt="Foto Produk">
...
const formInsert = ref({
    gambar: null,
    nama: props.detailData && props.method == 'edit' ? props.detailData.title : ''
})
...
const selectedImage = ref(
    ...
    ? baseImgUrl + props.detailData.images[0]
    : null
)
...
const submitForm = async() => {
    ...
    formData.append('title', dompurify.sanitize(formInsert.value.nama))
    ...
    try {
        ...
        const response = await initAPI('post', props.method == 'edit' ? `pages/${props.detailData.id}`  : 'pages', formData, token)
        localStorage.setItem('initNotif', JSON.stringify({type: 'Success', msg: response.data.message}))
        closeBtnModal()
        emit('sendNotif')
        refreshData()
    } catch (error) {
        toggleNotif('Error', 'Terjadi kesalahan saat menambah kategori.')
    } finally {
